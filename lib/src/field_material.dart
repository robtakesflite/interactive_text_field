...
/// A Material-themed [InteractiveTextField].
///
/// Reads defaults from the ambient [ThemeData] (cursor color, selection
/// color, text style, selection handles, context menu) so the field
/// matches the rest of a Material app without manual styling.
///
/// All parameters override the theme-resolved defaults when supplied.
class MaterialInteractiveTextField extends StatelessWidget {
  const MaterialInteractiveTextField({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedCursorColor = cursorColor ??
        selectionTheme.cursorColor ??
        theme.colorScheme.primary;
    final resolvedSelectionColor = selectionColor ??
        selectionTheme.selectionColor ??
        theme.colorScheme.primary.withValues(alpha: 0.4);
    final resolvedStyle = style ?? theme.textTheme.bodyLarge;
    final resolvedBackgroundCursorColor =
        backgroundCursorColor ?? const Color(0xFF999999);
